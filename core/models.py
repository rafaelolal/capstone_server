...
class Classroom(models.Model):
    ...
    period = CharField(max_length=6, choices=[
                       (c, c) for c in periods], unique=True)
...
class Unit(models.Model):
    key = ShortUUIDField(length=4, alphabet="012345679",
                         unique=True, primary_key=True)
    signed = BooleanField(null=True)
    classroom = models.ForeignKey(
        Classroom, related_name="units", on_delete=models.CASCADE, blank=True, null=True)
...
class Answer(models.Model):
    unit = models.ForeignKey(
        Unit, related_name="answers", on_delete=models.CASCADE)
...
class PeerReview(models.Model):
    unit = models.ForeignKey(
        Unit, related_name="peer_reviews", on_delete=models.CASCADE)
